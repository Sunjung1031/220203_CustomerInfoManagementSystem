 ...
 *    개발자   작성일            작업내용
 *    ----   ----            -------
 *    김선정   2022.02.03     인트로 화면 구현 중 메뉴, West쪽의 입력,신상정보검색 Border
 *    김선정   2022.02.04     인트로 화면 구현 중 SOUTH쪽,CENTER쪽, '추가'기능 구현  
 *    김선정   2022.02.07     '수정', '삭제' 기능 구현, '검색' 버튼 클릭시 정보검색 카드로 바꿈 
 *    김선정	 2022.02.08		'검색', '파일열기', '파일저장', '닫기' 기능구현 
 */

// 윈도우(GUI) 프로그램 개발 시 먼저 JFrame 클래스로부터 상속을 받자!!
// JFrame은 최상위 컨테이너이다.
public class ManageSystem extends JFrame {  //외부클래스 
         ...
         String passwd = "test1234";  //실제 패스워드 
          ...
          if(password == null) {
             ...
             JOptionPane.showMessageDialog(null, "패스워드가 맞지 않습니다."+"\n"+"'확인'버튼을 누르세요.",
                                        "패스워드 인증실패" , JOptionPane.ERROR_MESSAGE);
             continue OUTTER;
          }
      }
   }
...
//   MenuMain 클래스
   ...
   class MenuMain extends JPanel implements ActionListener, ItemListener {
      ...
      public void open() {
    	  ...
    	  try { 
    		...
    		while((line = read.readLine())!= null) {
    			//메모장에 이렇게 저장되어 있음 => 1, 홍길동, 010-1234-5678, · · ·, 프로그래머
      ...
      public void save() {
         ...
         try {
            ...
            for(int i=0; i<showTable.table.getRowCount(); i++) {
               ...
               // vector 객체에 저장 형태를 보면 [ 1, 홍길동, 010-1234-5678, · · ·, 프로그래머] 대과호 제거위해 -1한다.
      ...
      public void itemStateChanged(ItemEvent e) {
         if(e.getSource().equals(sno))  numSort();  //'번호'정렬 사용자 정의 메소드 
         else if(e.getSource().equals(sname))  stringSort(1);  //'이름'정렬 사용자 정의 메소드 
         else if(e.getSource().equals(schul))  stringSort(7);  //'출생지역'정렬 사용자 정의 메소드 
         else if(e.getSource().equals(sjob))  stringSort(9);  //'직업'정렬 사용자 정의 메소드 
      }
      //'번호'정렬 사용자 정의 메소드 구현
      public void numSort() {
    	  /*
    	   ...
    	   */
		int row = showTable.table.getRowCount();     //행의 갯수 얻어 옴 
		int col = showTable.table.getColumnCount();  //열의 갯수 얻어 옴
...
//   West 클래스
   ...
   class West extends JPanel {
      ...
      class Output extends JPanel implements ActionListener {
         ...
         @Override
         public void actionPerformed(ActionEvent e) {
            //[중요] searchType 값을 활용하여 search() 메소드에게 전달하여 하나의 메소드로 네 가지 검색기능을 해결하자. 
         ...
         public void search() {
            ...
            //순차검색(Sequential search 알고리즘 적용 
            for(int i=0; i<showTable.data.size(); i++) {
               
               //[아주 중요] 테크닉 -> 메소드 하나로 4가지 검색기능 실행  
            ...
            showTable.TableSize(); //셀크기 조절 메소드 호출 
...
//   Buttons 클래스
   ...
   class Buttons extends JPanel implements ActionListener {
      ...
      public Buttons() {
         ...
         //버튼들의 이벤트 연결 
         addBtn.addActionListener(this); //addAction의 add는 추가가 아닌 연결의 의미 
      ...
      public void actionPerformed(ActionEvent e) {
         if(e.getActionCommand().equals("추가")) addData(); //사용자 정의 메소드 호출  (이용자가 추가 버튼을 클릭시 addData처리)
      ...
      //'검색' 버튼에서 ActionEvent가 발생했을 때 처리하는 메소드 구현
      ...
      public void updateData() {
       ...
       //'직업' 수정시
        ...
        //데이터 "수정" 후 다음 데이터를 입력받기 위해서 주민번호 입력란 다시 활성화 시키자!! 
        west.input.tf[4].setEditable(true);
        west.input.tf[0].requestFocus(); //번호입력란에 포커스를 넣어라!!
      ...
      public void addData() {
        nextBtn.setEnabled(true); //'다음'버튼 활성화 
        vector = new Vector<String>();   //고객관리시스템은 계속 정보가 추가 되므로 가변배열인 vector사용 
        ...
        //juminNum이 가리키는 주민번호를 체크 공식 적용하여 체크하자!!
        int[] weight = {2,3,4,5,6,7,0,8,9,2,3,4,5}; //가중치 배열준비 
        int sum = 0;
        int temp, result; 
        
        String regex = "[0-9]{6}-[1234][0-9]{6}"; //'주민번호' 정규표현식(regex)적용 => 유효성체크 
        ...
        if(check ==  false) {
           ...
           return; //그 상태로 그대로 있어라!!
              ...
              for(int i=0; i<juminNum.length()-1; i++) {
                 if(juminNum.charAt(i)=='-')continue;
                   sum += weight[i]*(juminNum.charAt(i)-48); 
              }
              //[2단계, 3단계 공식적용]
              temp = 11 -(sum%11);
              result = temp%10;
              ...
              if(result == juminNum.charAt(13)-48) {
                 JOptionPane.showMessageDialog(null, 
                       "주민번호 체크 결과 정상입니다."+"\n"+"'확인'버튼을 누르면 정보가 출력됨!!");
                 vector.add(juminNum); //주민번호가 정상이기 때문에 Vector객체에 추가시킨다.
           ...
           //사용자가 선택한 직업 얻어내기
           //getSelectedItem()메소드는 리턴타입이 Object이기 때문에 String 타입으로 변환하여 리턴해야함!
